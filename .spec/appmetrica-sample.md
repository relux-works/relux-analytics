...
- The sample app uses `ReluxAnalyticsAppMetrica` as an AppMetrica-backed `Analytics.Service`.
- The AppMetrica service initializes AppMetrica with `AppMetricaConfiguration(apiKey:)` and `AppMetrica.activate(with:)`.
- UI and flow code emit analytics through `Analytics.Effect.track(.instant(...))` and `Analytics.Effect.track(.continuous(...))`, not through direct AppMetrica SDK calls.
- UI and flow code emit continuous start/update/stop events. `ContinuousEventAggregate` is produced only by the SDK continuous event manager.
