...
The aggregate is readable by collector sinks, but its constructor is not part of the public SDK surface.
...
- `handle(.start)` creates active span state by `SpanID`.
- If `handle(.start)` receives an already active `SpanID`, it emits a `.replaced` aggregate for the previous span before storing the new span.
